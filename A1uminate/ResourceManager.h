...
public:
	static void SetCallRefCallback(const std::function<std::string(const std::string&, const std::string&)>& refCallback);
...
public:
	...
	//
	static fwEvent<ResourceManager*> OnInitializeInstance;

	//
	// Gets the current resource manager.
	//
	static ResourceManager* GetCurrent(bool allowFallback = true);
};
